...
export const generateYAxis = (revenue: Revenue[]) => {
  ...
  const devidedBy = highestRecord / 10;

  for (let i = topLabel; i >= 0; i -= devidedBy) {
    if (topLabel / 100 / 1000 < 10) {
      yAxisLabels.push(`$${(i / 100 / 1000).toFixed(1)}K`);
    } else {
      yAxisLabels.push(`$${Math.floor(i / 100 / 1000)}K`);
    }
  }
